CROPS.optp <- function(data,min_pen=5,max_pen=20,sigsquared=1) {
  ...
  while (length(pen_interval) > 0){
    ...
    for (b in 1:length(pen_interval)) {
      #browser()
      ans <- optp(data,pen_interval[b],sigsquared) 
      resultingcpts <- ans[[2]]##c(0,ans[[2]],n)
      new_numcpts[b] <- length(resultingcpts)
      cost.test <- array()
      new_cpts[b] <- list(resultingcpts)
      new_penalty[b] <- ans[[1]]-(length(resultingcpts)-2)*pen_interval[b]
    }
